...
CHECKER_TASK_PROMPT = (
    "You are ProtocolCheckerGPT. Your task is to look at the provided protocol and determine if it is expressive "
...
class ProtocolPicker:
    ...
    def check_protocol_for_task(
        self, protocol_document: str, task_schema: TaskSchemaLike
    ) -> bool:
        ...
        conversation = self.toolformer.new_conversation(
            CHECKER_TASK_PROMPT, [], category="protocolChecking"
        )
    ...
    def pick_protocol(
        self, task_schema: TaskSchemaLike, *protocol_lists: List[Protocol]
        ...
        for protocol_list in protocol_lists:
            for protocol in protocol_list:
                if self.check_protocol_for_task(
                    protocol.protocol_document, task_schema
                ):
