...
class SanitizedInterpreter(BaseInterpreter):
    ...
    def _execute_ast(self, expression: ast.AST) -> Any:
        if isinstance(expression, ast.Assign):
            ...
            # Function definition -> define it and add it to the state
            def func(*args, **kwargs):
                # Use introspection to get the args names

                # Get the function signature
                signature = expression.args
            ...
            try:
                ...
                for handler in expression.handlers:
                    if isinstance(e, self._execute_ast(handler.type)):
